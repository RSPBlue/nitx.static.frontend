...
const IdealizationDiagram = () => {
  const { links, idealization } = labels

  const allCards = [...idealization.cardsLeft, ...idealization.cardsRight]

  return (
    <PageSection id={links.idealization}>
      <Title>
        {idealization.thinTitle} <strong>{idealization.boldTitle}</strong>
      </Title>
      <p className="border-2 border-brand rounded-xl text-sm p-4">
        {idealization.subtitle}
      </p>
      {/* Mobile to 1280px */}
      <div className="flex flex-wrap gap-10 w-full overflow-x-hidden font-oswald font-extrabold text-brand text-[36px] xl:hidden">
        {allCards.map(({ title, subtitle, icon }) => (
          <IdealizationMobileCard key={title} title={title} Icon={icon}>
            {subtitle}
      ...
      <ArcherContainer lineStyle="straight" className="w-full hidden xl:block">
        <div className="flex w-full h-[500px] px-20">
          <div className="flex-1 flex text-[36px] justify-around flex-col-reverse h-full gap-4 font-oswald font-extrabold text-brand">
            ...
            {idealization.cardsLeft.map(
              ({
                ...
                const position = archerId.split("-").at(2) ?? "0"

                return (
                  <div key={archerId} className="flex flex-col">
                    <ArcherElement
                      id={archerId}
                      relations={getIdealizationArcherRelations(
                        "left",
                        +position,
                      )}
                    >
                      <div
                        style={{ paddingLeft: `${paddingTitle}px` }}
                        className="w-fit text-end pr-4"
                      >
                        {title}
                      </div>
                    </ArcherElement>
                    <span
                      style={{ paddingLeft: `${paddingSubtitle}px` }}
                      className="font-unbounded text-xs font-light text-foreground w-1/2"
                    >
                      {subtitle}
                    </span>
                  </div>
            ...
            {/* DIV PARA MANTER O CONTEÚDO POSICIONADO MAISD ABAIXO */}
          ...
          <div className="flex-1 flex text-[36px] justify-around flex-col items-end h-full gap-4 font-oswald font-extrabold text-brand">
            {/* CARDS RIGHT */}
            {idealization.cardsRight.map(
              ({ title, subtitle, paddingTitle, archerId }) => {
                const position = archerId.split("-").at(2) ?? "0"
                ...
                return (
                  <div key={archerId} className="flex flex-col">
                    ...
                    >
                      <div
                        style={{ paddingRight: `${paddingTitle}px` }}
